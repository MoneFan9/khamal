...
class USBMountManager:
    ...
    def _validate_paths(device_path, mount_point) -> tuple[bool, str, str]:
        ...
        if os.path.commonpath(["/dev", device_path]) != "/dev":
            ...
            return False, device_path, mount_point_normalized

        allowed_mount_base = os.path.normpath("/mnt/usb")
        try:
            if os.path.commonpath([allowed_mount_base, mount_point_normalized]) != allowed_mount_base:
                logger.error(f"Invalid mount point: {mount_point_normalized}. Must be within {allowed_mount_base}")
                return False, device_path, mount_point_normalized

            if mount_point_normalized == allowed_mount_base:
                logger.error(f"Cannot mount directly on {allowed_mount_base}")
                return False, device_path, mount_point_normalized
        except ValueError:
            logger.error(f"Invalid paths for commonpath: {allowed_mount_base}, {mount_point_normalized}")
            return False, device_path, mount_point_normalized

        return True, device_path, mount_point_normalized
    ...
    def mount_volume(device_path, mount_point):
        ...
        is_valid, device_path, mount_point_normalized = USBMountManager._validate_paths(device_path, mount_point)
        if not is_valid:
            return False
        ...
        try:
            ...
            logger.error(f"Could not stat device {device_path}")
            return False
        ...
        if not os.path.exists(mount_point_normalized):
            try:
                os.makedirs(mount_point_normalized, exist_ok=True)
            except OSError as e:
                logger.error(f"Failed to create mount point {mount_point_normalized}: {e}")
                return False
        ...
        try:
            command = ["sudo", "mount", "-o", mount_options, device_path, mount_point_normalized]
            subprocess.run(command, check=True, capture_output=True, text=True)
            logger.info(f"Successfully mounted {device_path} to {mount_point_normalized} with security options.")
            return True
        except subprocess.CalledProcessError as e:
            logger.error(f"Failed to mount {device_path} to {mount_point_normalized}: {e.stderr}")
            return False

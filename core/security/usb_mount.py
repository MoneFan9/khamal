...
class USBMountManager:
    ...
    def _validate_paths(device_path, mount_point) -> tuple[bool, str, str]:
        ...
        Security Logic:
        1. Normalizes paths to prevent directory traversal attacks.
        2. Restricts device paths to /dev.
        3. Restricts mount points to /mnt/usb/ to prevent overwriting system directories.
        """
        try:
    ...
    def mount_volume(device_path, mount_point):
        ...
        # Ensure it's a block device to avoid mounting regular files or symbolic links to system files.
        ...
        # The pattern ensures the device path is authorized by a specific 'allow' rule in USBGuard.
        ...
        try:
            # We use sudo for mounting, assuming the system is configured to allow this for the khamal user via sudoers.

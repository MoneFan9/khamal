...
class USBMountManager:
    ...
    def mount_volume(device_path, mount_point):
        ...
        # Essential to prevent mounting regular files or character devices as volumes.
        if not Path(device_path).is_block_device():
            logger.error(f"Security Policy Violation: {device_path} is not a valid block device.")
            return False

        # 3. Secure Mounting Environment
        # Reserved for future SELinux/AppArmor context enforcement.
        # Currently enforced via mount options (noexec, nosuid, nodev).

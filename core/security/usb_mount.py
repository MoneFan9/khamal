...
class USBMountManager:
    ...
    def _validate_paths(device_path, mount_point) -> tuple[bool, str, str]:
        ...
        """
        try:
            # 1. Use realpath to resolve symlinks and prevent bypasses
    ...
    def mount_volume(device_path, mount_point):
        ...
        # 1. Basic Block Device Verification
        try:
        ...
        # and ensure 'with-devpath' is used for strict mapping.
        path_pattern = re.compile(rf"\ballow\b.*with-devpath\s+\"({re.escape(device_path)}|{re.escape(parent_device)})\"")

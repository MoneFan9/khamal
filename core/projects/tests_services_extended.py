...
@pytest.fixture
def test_user(db):
    return User.objects.create_user(username="testuser_ext", password="password")
...
class TestServicesExtended:
    ...
    def test_ensure_project_network_recreate(self, mock_get_client, test_user):
        project = Project.objects.create(name="testproj_recreate", owner=test_user, network_id="old-id")
    ...
    def test_ensure_project_network_exists_by_name(self, mock_get_client, test_user):
        project = Project.objects.create(name="testproj_byname", owner=test_user)
    ...
    def test_ensure_project_network_failure(self, mock_get_client, test_user):
        project = Project.objects.create(name="testproj_fail", owner=test_user)
        mock_client = MagicMock()
        mock_get_client.return_value = mock_client
        mock_client.networks.list.return_value = []
    ...
    def test_container_lifecycle_exceptions(self, mock_get_client, test_user):
        project = Project.objects.create(name="testproj_lifecycle", owner=test_user)
    ...
    def test_lifecycle_no_id(self, test_user):
        project = Project.objects.create(name="testproj_noid", owner=test_user)
        deployment = Deployment.objects.create(project=project, container_id=None)
        project_no_net = Project.objects.create(name="nonet", owner=test_user, network_id=None)
    ...
    def test_delete_project_network_failure(self, mock_get_client, test_user):
        project = Project.objects.create(name="testproj_netfail", owner=test_user, network_id="net-id")
        mock_client = MagicMock()
        mock_get_client.return_value = mock_client
    ...
    def test_get_routing_labels_ssl(self, test_user):
        project = Project.objects.create(name="testproj_ssl", owner=test_user, domain="example.com")
    ...
    def test_get_routing_labels_no_ssl(self, mock_settings, test_user):
        project = Project.objects.create(name="testproj_nossl", owner=test_user, domain="example.com")
    ...
    def test_get_deployment_volumes_hot_reload(self, test_user):
        project = Project.objects.create(name="testproj_hot", owner=test_user)
    ...
    def test_get_deployment_volumes_missing_localsource(self, test_user):
        project = Project.objects.create(name="testproj_nohot", owner=test_user)
        deployment = Deployment.objects.create(project=project, hot_reload=True)
    ...
    def test_provision_database_existing_running(self, mock_ensure_net, mock_get_client, test_user):
        project = Project.objects.create(name="testproj_dbexist", owner=test_user)
    ...
    def test_provision_database_existing_stopped(self, mock_ensure_net, mock_get_client, test_user):
        project = Project.objects.create(name="testproj_dbstopped", owner=test_user)
        mock_ensure_net.return_value = "net-id"
    ...
    @patch("projects.services.get_docker_client")
    @patch("projects.services.ensure_project_network")
    def test_provision_database_general_exception(self, mock_ensure_net, mock_get_client, test_user):
        project = Project.objects.create(name="testproj_dbfail", owner=test_user)
        mock_ensure_net.return_value = "net-id"
    ...
    def test_provision_database_not_healthy(self, mock_wait, mock_ensure_net, mock_get_client, test_user):
        project = Project.objects.create(name="testproj_dbunhealthy", owner=test_user)
        mock_ensure_net.return_value = "net-id"
    ...
    def test_provision_database_race_condition(self, mock_ensure_net, mock_get_client, test_user):
        project = Project.objects.create(name="testproj_race", owner=test_user)
        mock_ensure_net.return_value = "net-id"
    ...
    def test_get_deployment_logs_not_found(self, mock_get_client, test_user):
        project = Project.objects.create(name="testproj_logs", owner=test_user)

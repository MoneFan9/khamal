...
def _get_traefik_config() -> tuple[list[str], dict[str, dict]]:
    ...
    Traefik connects to the Docker API via 'docker-socket-proxy' over TCP.
    This prevents Traefik from having direct access to /var/run/docker.sock.
    """
    command = [
    ...
    # No more direct docker.sock mount
    volumes = {}

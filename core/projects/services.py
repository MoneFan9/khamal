...
def ensure_project_network(project: Project) -> str:
    ...
    if project.network_id:
        try:
            ...
            logger.error(f"Docker API error when checking network {project.network_id}: {e}")
    ...
    try:
        ...
        if e.response is not None and e.response.status_code == 409:
            ...
            # Retrieve the existing network ID
            networks = client.networks.list(names=[network_name])
            if not networks:
                # This should theoretically not happen if 409 was returned
                raise
            network = networks[0]
        else:
...
def provision_database(project: Project, engine: str):
    ...
    try:
        ...
        logger.error(f"Docker API error when checking database container {container_name}: {e}")

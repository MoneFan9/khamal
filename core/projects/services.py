...
def ensure_project_network(project: Project) -> str:
    ...
    if project.network_id:
        try:
            ...
            logger.warning(f"Network {project.network_id} for project {project.name} not found or inaccessible, recreating.")
    ...
    try:
        ...
        if e.response is not None and e.response.status_code == 409:
            logger.info(f"Network {network_name} already exists (race condition or cleanup).")
            # Retrieve existing network ID
            networks = client.networks.list(names=[network_name])
            if networks:
                network = networks[0]
            else:
                raise # Should not happen with 409
        else:

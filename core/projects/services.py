...
def _setup_deployment_container_metadata(deployment: Deployment) -> tuple[dict, dict, str]:
    """
    Sets up labels, volumes, and network ID for a deployment container.
    """
    ensure_global_proxy()
    ...
    return labels, volumes, project_network_id
...
def create_deployment_container(deployment: Deployment, image: str):
    ...
    # 1. Setup metadata (networks, labels, volumes)
    labels, volumes, project_network_id = _setup_deployment_container_metadata(deployment)
    ...
    try:
        ...
        # 3. Connect to global proxy network for routing

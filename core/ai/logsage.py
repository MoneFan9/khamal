...
class LogSagePreprocessor:
    ...
    def _gen_deduplicate(self, iterable):
        """
        Deduplicates consecutive lines using a generator to save memory.
        """
        prev = None
    ...
    def process(self, raw_logs: str) -> List[str]:
        ...
        Optimized for memory by using io.StringIO and a safety cap of 2000 lines.
        """
        if not raw_logs:
            return []

        # Use StringIO for memory-efficient log line iteration
        log_stream = io.StringIO(raw_logs)

        # Use generator expressions and islice to enforce a safety cap of 2000 lines
        # This prevents OOM on massive log files.
        ...
        # Convert to list for prioritization (requires multiple passes)

...
class LogSagePreprocessor:
    ...
    def process(self, raw_logs: str) -> List[str]:
        ...
        Uses generators for memory efficiency and implements a safety cap for 8GB RAM constraints.
        """
        if not raw_logs:
            return []
        ...
        # Use io.StringIO to avoid creating large intermediate lists of lines
        log_stream = io.StringIO(raw_logs)

        # 1. Use generator expressions to reduce memory overhead
        # 2. Implement a 2000-line safety cap to prevent OOM on massive log bursts
        lines = (line.strip() for line in islice(log_stream, 2000) if line.strip())
        filtered = (line for line in lines if not self.is_noise(line))

        # Deduplicate using a generator-friendly approach
        def _gen_deduplicate(iterable):
            prev = None
        ...
        deduplicated_gen = _gen_deduplicate(filtered)

        # Convert to list for prioritization logic (multi-pass indexed access)
